...
//            }
    ...
    private void handleConnection(Socket socket) throws IOException {
        ...
        //for problem5
        out.println("Welcome to Minesweeper. Players: " +this.playersCounter + " including you. ");
        out.println("Board: " +this.sizeCols+ " " + " columns by " +this.sizeRows+ " Rows");
        out.println("Type 'help' for help.");

        try {
            for (String line = in.readLine(); line != null; line = in.readLine()) {
                String output = handleRequest(line);
                if (output != "disconnect" && output != "BOOM!\r\n") {
                    // TODO: Consider improving spec of handleRequest to avoid use of null
                    out.println(output);
                }
                else if(output == "BOOM!\r\n")
                {
                	//System.out.println("Exit strat hitting");
                	out.println(output);
                	break;
                }
                else
           ...
           System.out.println("Finally hitting after bye");
        	out.close();
            in.close();
        }
    }
    ...
    private String handleRequest(String input) {
        ...
        if ( ! input.matches(regex)) {
            // invalid input
            String help = "Valid Commands: \"look\", \"dig X,Y\", \" flag X,Y\", \"deflag X,Y\"\n";
            return help;
        }
        ...
        if (tokens[0].equals("look")) {
            ...
            if (tokens[0].equals("dig")) {
                ...
                // TODO Problem 5
            	String bombFlag = "";
            	if(x >= 0 && x < this.sizeCols && y >= 0 && y < this.sizeRows)
            	{
            		
            		bombFlag += this.board.digSquare(y, x);
            	}
                ...
                if(x >= 0 && x < this.sizeCols && y >= 0 && y < this.sizeRows)
                {
                	if(!this.board.getSquare(y, x).checkFlagged())
                	{
                		this.board.toggleFlagASquare(y, x);
                	}
    ...
    public static void runMinesweeperServer(boolean debug, Optional<File> file, int sizeX, int sizeY, int port) throws IOException {
        ...
        if(file.isPresent())
        {
        	...
        	server.board = new Board(file.get().toString());
        	
        	//this.sizeCols = board.getColsSize();
        	//this.sizeRows = board.getRowsSize();
        	server.sizeCols = server.board.getColsSize();
        	server.sizeRows = server.board.getRowsSize();
        }
        else
        {
        	//this.sizeCols = sizeX;
